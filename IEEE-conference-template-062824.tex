...
Empirical evaluation demonstrates that EMG features, specifically Variance (VAR) and Root Mean Square (RMS), are the primary predictors for movement classification, outperforming traditional kinematics. During live testing, the localized 
...
This paper addresses these gaps by proposing a multi-modal, dual-tier architecture that fuses Inertial Measurement Units' (IMU) kinematics with Electromyography (EMG). By leveraging Edge Artificial Intelligence for 
...
electromyography (sEMG) and inertial measurement units (IMU) to classify physical fatigue during natural over-ground walking. Their hybrid Convolutional Neural Networks-Long Short-Term Memory (CNN-LSTM)-Attention 
model, evaluated via leave-one-subject-out cross-validation, achieved 87.94\% accuracy, proving that combining physiological markers from the gastrocnemius with kinematic jerk signals significantly 
...
feature extraction (RMS, MAV, median frequency (MDF)) to classify muscle fatigue during sustained isometric tasks. By comparing logistic regression, decision trees (DT), and XGBoost, they identified 
...
divided into three primary layers: the sensor-embedded hardware, the mobile edge-computing hub, and the cloud-based inference server highlighted in yellow, blue and green respectfully in Fig.~\ref{systemOverview}.

\begin{figure*}[htbp]
\centerline{\includegraphics[width=0.9\textwidth]{System_Overview.jpg}}
...
As illustrated in Fig.~\ref{systemOverview}, the architecture facilitates a high-bandwidth data pipeline that transitions from raw physical signals to actionable biofeedback. The process is categorized into three 
distinct operational stages:

\begin{enumerate}
\item \textbf{Acquisition Layer:} The footwear hardware captures bilateral IMU and EMG signals, calculates six EMG features, and sends the data to the next layer through BLE.
\item \textbf{Edge Computing Hub:} The Android application serves as the connection point between the user, the prototypes and the cloud. It handles the simultaneous asynchronous streams from both devices, performing temporal synchronization to align the 15 features from each foot into a unified 30-feature vector. This is where the cramping warning and the localized TinyML inference occur, allowing for "offline" movement recognition and immediate feedback to the athlete.
\item \textbf{Cloud and Analytical Layer:} The mobile app forwards the synchronized data to a cloud infrastructure. Data is stored in a time-series database (InfluxDB) for historical visualization, while the Flask-based server executes the high-dimensional Random Forest model. 
\end{enumerate}
...
To optimize BLE bandwidth, the system bundles five 30-byte frames into a single 150-byte payload, transmitted to the mobile hub every 100ms.


\begin{figure}[b]
\centerline{\includegraphics[width=0.9\linewidth]{deviceTop.jpg}}
...
To handle the inherent mechanical noise of high-intensity basketball movements, the system utilizes a 300ms sliding window with a 100ms step size for feature aggregation. A critical contribution of this 
...
local predictions, providing a 500ms context. This logic acts as a digital low-pass filter, effectively eliminating prediction "flickering" and hardware-induced false positives, thereby ensuring a stable and 
...
\begin{itemize}
  ...
  \item Cloud Tier (Random Forest): Simultaneously, the 300ms window is serialized into a JSON payload and transmitted via an asynchronous HTTP POST request to a Flask-based server. The server hosts a high-dimensional Random Forest (RF) classifier used for deep analysis and comparative validation against the edge model.
\end{itemize}
...
athlete executed specific basketball movements while researcher chose the correct label and turned a "Send data" switch within the Android User Interface (UI).
...
high generalization. Conversely, the Cloud-based Random Forest (RF) was evaluated using Receiver-Operating Characteristic (ROC) and Precision-Recall (PR) curves. The Area Under the Curve (AUC) for all classes 
...
A core contribution of this work is the validation of EMG integration. Gini Feature Importance analysis (Fig.~\ref{featureImportance}) revealed that EMG-derived features, specifically VAR and RMS, were the primary 
contributors to classification accuracy. This confirms that physiological electrical activity provides critical intent-based 
...
\bibitem{b1} Lutz, Jonas \& Memmert, Daniel \& Raabe, Dominik \& Dornberger, Rolf \& Donath, Lars. (2019). Wearables for Integrative Performance and Tactic Analyses: Opportunities, Challenges, and Future Directions. International Journal of Environmental Research and Public Health. 
\bibitem{b6} Hwang, Soree \& Kwon, Nayeon \& Lee, Dongwon \& Kim, Jongman \& Yang, Sumin \& Youn, Inchan \& Moon, Hyuk-June \& Sung, Joon-Kyung \& Han, Sungmin. (2025). A Multimodal Fatigue Detection System Using sEMG and IMU Signals with a Hybrid CNN-LSTM-Attention Model. Sensors.  
\bibitem{b9} Otálora S, Segatto MEV, Monteiro ME, Múnera M, Díaz CAR, Cifuentes CA. Data-Driven Approach for Upper Limb Fatigue Estimation Based on Wearable Sensors. Sensors (Basel). 2023 Nov 20. doi: 10.3390/s23229291.
